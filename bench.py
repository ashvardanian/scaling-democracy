"""Benchmarks every Schulze backend against each other, and reports the election it decided.

Backends are selected by regex against their names, so `-k GPU` runs both GPU rows and
`-k Hopper` runs only the bulk-tensor one. The first that runs becomes the baseline the
rest are validated against.

    python bench.py --num-candidates 4096 --num-voters 4096
    python bench.py --num-candidates 2048 --num-voters 0 -k GPU --warmup 1 --repeat 20

Flags, with their defaults:

    -k, --filter REGEX    which backends run, matched against their names, `.` for all
    --num-candidates N    alternatives to rank, 128
    --num-voters N        ballots to tally, 2000, or 0 to draw the matrix directly
    --warmup N            iterations to discard, 1
    --repeat N            timed iterations to average, 1
    --seed N              seeds the preference generator, 42

The tile edge is not a flag. It is fixed at compile time to 32, which fits a CPU L2 slice
and matches an NVIDIA warp; edit `tile_size_k` in `types.cuh` or `TILE_SIZE` in
`schulze.py` and rebuild to compare. A tile wider than the electorate is not an error:
the tail is zero-filled, and zero is the identity of the max-min semiring.

Backend names are `Serial (Numba)`, `Tiled CPU (Numba)`, `Tiled CPU (OpenMP)`, `Tiled GPU`
and `Tiled GPU (Hopper)`. See `cli.mojo` for the Mojo driver, which takes the same flags.
"""
...
def time_one_call(callback: Callable[[np.ndarray], np.ndarray], preferences: np.ndarray) -> tuple[float, np.ndarray]:
    """One call of a backend, returning its wall clock in seconds alongside its result."""
...
def benchmark_implementation(
    ...
    """Times `repeat` calls after `warmup` discarded ones, returning the mean seconds and the last result."""
    if repeat < 1:
    ...
    for iteration in range(warmup):
        elapsed_time, _ = time_one_call(callback, preferences)
        iteration_note = f" {iteration + 1}/{warmup}" if warmup > 1 else ""
        print(f"  Warm-up{iteration_note}: {format_time(elapsed_time)}")

    total_seconds, result = time_one_call(callback, preferences)
    for _ in range(repeat - 1):
        elapsed_time, result = time_one_call(callback, preferences)
        total_seconds += elapsed_time
    return total_seconds / repeat, result
...
if __name__ == "__main__":
    ...
    backends = [
        ("Serial (Numba)", compute_strongest_paths_numba_serial),
        ("Tiled CPU (Numba)", compute_strongest_paths_numba_parallel),
        ("Tiled CPU (OpenMP)", partial(compute_strongest_paths, backend="cpu_openmp")),
        ("Tiled GPU", partial(compute_strongest_paths, backend="gpu_serial")),
        ("Tiled GPU (Hopper)", partial(compute_strongest_paths, backend="gpu_hopper")),
    ]
    ...
    # the caller's back.
    if baseline is None or baseline_callback is None:
